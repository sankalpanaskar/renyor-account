...
import { CustomCounselingButtonComponent } from './custom-counseling-btn.component';
import { LocalDataSource } from 'ng2-smart-table';
import { InterviewQuestionsComponent } from '../interview-questions/interview-questions.component';
import { CustomStudentButtonComponent } from './custom-student-btn.component';
import { StudentAnswersComponent } from '../student-answers/student-answers.component';

@Component({
  selector: 'ngx-manage-counseling',
  templateUrl: './manage-counseling.component.html',
  styleUrls: ['./manage-counseling.component.scss']
})
export class ManageCounselingComponent implements OnInit {
  ...
  settings = {
    ...
    columns: {
      ...
      //     if (cell === 1) {
      //       return `<h6><span class="badge rounded-pill bg-primary text-white pl-2 pr-2 custom-badge">Done</span></h6>`;
      //     } 
      //     else if (cell === 0) {
      ...
      // },
      // action: {
      //   title: 'View Student Form',
      //   type: 'custom',
      //   renderComponent: CustomStudentButtonComponent,
      //   filter: false,
      //   onComponentInitFunction: (instance) => {
      //     instance.editClicked.subscribe((row) => {
      //       console.log('Row passed from CustomStudentButtonComponent[]:', row);
      //       this.openEditDialogStudent(row); // 👈 Send full original object
      //     });
      //   }
      // },
      phone: {
      ...
      action2: {
        ...
        type: 'custom',
        width: '80px',
        renderComponent: CustomCounselingButtonComponent,
        filter: false,
        onComponentInitFunction: (instance) => {
          instance.editClicked.subscribe((row) => {
  ...
  loadCenters() {
    var data = {
      role: this.globalService.role_id,
      user_id: this.globalService.user_id
    }
    this.globalService.getCenterFilter(data).subscribe({
      next: (res) => {
  ...
  loadStudentList(centerId) {
    this.loading = true;
    this.globalService.getStudentList(centerId).subscribe({
      next: (res) => {
  ...
  // studentChange(status: any) {
  //   let filteredData;

  //   if (status === 'all') {
  //     filteredData = this.apiData;
  //   } else {
  //     filteredData = this.apiData.filter(
  //       student => student.student_counseling_status === +status
  //     );
  //   }
  ...
  //   } else {
  //     filteredData = this.apiData.filter(
  //       student => student.po_counseling_status === +status
  //     );
  //   }
